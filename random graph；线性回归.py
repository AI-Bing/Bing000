...
from scipy.linalg import block_diag #计算二阶导，块矩阵拼接需要用到block_diag命令
...
###生成系数数据 uim，data_uim是一个列表，列表中每一个元素是一个1000*3的矩阵。
data_uim = []
i=j=k=0
for i in range(n):
    np.random.seed(i) #注意添加np.，否则seed可能无效。
    uim=np.random.normal(0,10**0.5,size=(data_m,N))#生成一个1000*3的多位数组，均值为0，方差为10。
    uim[:,N-1]=1#将第三列改为1，论文设定。
    data_uim.append(uim)
###生成数据 cim，data_cim是一个列表，列表中每一个元素是一个1*1000的矩阵。
data_cim=[]
for k in range(n):
    np.random.seed(n+k)
    cim=np.random.normal(0,10**0.5,size=(1,data_m))
    data_cim.append(cim)
###生成数据 vim，data_vim是一个列表，列表中每一个元素是一个1*1000的矩阵。
data_vim=[]
for j in range(n):
...
###利用集中式梯度下降求解f的最优值及最优的优化变量x_0
if __name__ == '__main__':
    iter1=5000#迭代次数
    x1=np.array([1,1,1])#第一次迭代的初值
    r=0.00004#步长
    plt_loss_0=[]#记录loss值。
...
plt.plot(x_axix[0:iter0:80],np.array(plt_loss_1/loss1)[0:iter0:80], color='green', label='DGD',marker='s')
plt.plot(x_axix[0:iter0:80],np.array(plt_loss_2/loss1)[0:iter0:80], color='m', label='EXTRA',marker='o')
plt.plot(x_axix[0:iter0:80],np.array(plt_loss_3/loss1)[0:iter0:80], color='orange', label='D-NG',marker='d')
plt.plot(x_axix[0:iter0:80],np.array(plt_loss_4/loss1)[0:iter0:80], color='blue', label='Acc-DNGD-SC',marker='^')
plt.plot(x_axix[0:iter0:80],np.array(plt_loss_5/loss1)[0:iter0:80], color='red', label='DHB',marker='*')
plt.legend() # 显示图例
plt.xlabel(r'$k$')
plt.ylabel('Residual')
plt.yscale('log')
plt.savefig('(a) Random graph（1）.eps', dpi=500, bbox_inches='tight')
plt.show()